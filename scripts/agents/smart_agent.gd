...
func get_move():
	...
	var map_mid = controller.game_map.map_size
	map_mid.x = map_mid.x / 2
	map_mid.y = map_mid.y / 2

	var move = csharp_agent.GetMove(params[0], params[1], params[2], map_mid, \
...
func get_csharp_params() -> Array:
	...
	var other_units = controller.units_array.filter(func(u): 
		...
		return last_seen != -1 and last_seen < len(controller.units_array) * 2)\
		.map(func (u):
